...
Sesion 9 - Investigacion vi0/no en clon4 (bug de umbral invertido), cierre completo.

Fer pregunto por que las alertas vi0/no de clon4 parecian estar fijas en 80.1 en el historico completo (464 disparos), mientras que en teoria la formula deberia variar. Confirme via el dropdown Condicion de cada alerta la version exacta ligada (CLON3 = BB COMPLETO v178, CLON4 = VI-0 CLON4 v30, boot = BB COMPLETO v176 sin logica no todavia). Comparacion linea por linea de la formula vi0/no entre CLON3 y CLON4: son identicas caracter por caracter (temaSignal_no, kstSignalAdj_no, stochRsiSignal_no, ulcerSignal_no, zigzagSignal_no, sumadas y multiplicadas por 100/5), por lo que matematicamente solo pueden dar multiplos de 20 (0,20,40,60,80,100). Verifique en el Log real de TradingView que el mismo dia CLON3, CLON4, CLON14 y boot dispararon el mismo vi0/no/VI-0/80.1 al mismo segundo, confirmando que no es exclusivo de clon4.

Code habia reportado antes que clon3 mostraba valores 85.1 y 86.2, aparente contradiccion con la formula discreta. Verificamos la fuente exacta: resultaron ser 2 registros del 4 de mayo, apenas horas despues del primer despliegue de la logica vi0/no (version 177 de BB COMPLETO, 3 de mayo), y ya venian marcados DUPLICADO en la base de datos, es decir artefactos de un arranque, no una senal real de otro diseno. Revise el historial de versiones de BB COMPLETO y confirme que la version 176 (25 abril) no tenia logica vi0/no en absoluto, y que la version 177 (3 mayo) nace ya con esta misma formula discreta, por lo que el diseno de 6 niveles fue siempre asi, no perdio granularidad de un diseno anterior mas rico. Con esto Fer y Code cerraron esa duda.

El problema real, separado del anterior segun aclaro Fer (no son excluyentes): clon4 tenia el umbral de vi0 para el caso no invertido en el servidor (clon4/CONFIG.py, UMBRALES_INDICADORES vi0). clon3 y otros 5 bots (clon6, clon8, clon9, clon10, clon14) usan correctamente umbral_no 79 y condicion mayor que, mientras que clon4 tenia umbral_no 20 y condicion menor que, por lo que 80.1 nunca podia pasar. Esto causo que clon4 rechazara el 100 por ciento de sus votos bajistas de vi0 desde que existe esa logica: 519 rechazos historicos desde el 12 de mayo, 30 en las ultimas 24 horas, activo hasta el momento de la investigacion. Boot, clon2, clon12 y clon13 usan un mecanismo distinto (un solo umbral compartido de 80 para si y no) que no genera el mismo problema.

Fer autorizo directamente a Code aplicar el fix en su propia sesion (backup de CONFIG.py, cambio puntual de umbral_no a 79 y condicion_no a mayor que, py_compile, verificacion de en_posicion antes de reiniciar). Este cambio de servidor fue responsabilidad y ejecucion de Code, con autorizacion explicita de Fer dada directamente a Code en su sesion compartida. Yo no ejecute ni aprobe ningun comando de servidor en este hilo, solo la verificacion del lado de Pine y el historial de versiones. Fer aclaro que la descoordinacion entre boot/servidor y TradingView es un tema que le corresponde resolver a Code, no a esta sesion.


Sesion 9 - continuacion: verificacion de los 2 puntos pendientes marcados por Code (clon14 bb1 y clon6 mac), restringidos al ultimo mes por pedido de Fer. Punto 1, clon14 bb1: se verifico que la alerta CLON14 == BB1=VIO esta correctamente apuntada a BB COMPLETO V10.19.7.23 version 178, la misma que usa CLON3, sin ninguna advertencia de configuracion. Se confirmo ademas que esa alerta fue creada recien el mismo dia 22 de julio a las 13:42:31, coincidiendo con que clon14 es un bot recien puesto en marcha (activo desde el 21 de julio segun Code). Su primera y unica activacion registrada fue ese mismo dia a las 19:50:16, con el mensaje bb1/no/BB+1/100.1 y estado Webhook enviado correctamente en el Log. La condicion de disparo de ese lado, porcentaje mayor a 0.4 y normal mayor a 95, es una condicion extrema que ocurre pocas veces. Conclusion: no es un bug de Pine, los cero votos previos se explican por ser una alerta nueva combinada con una condicion rara, no por una falla tecnica del lado de TradingView.

Punto 2, clon6 mac: se confirmo un bug real. La alerta CLON 6 <<< MACD, que deberia escuchar el indicador MACD V10 V2, esta en realidad apuntada a la Condicion MIN TEN CLON6, exactamente el mismo script que usa la alerta separada CLON 6 <<< MIN-TEN. Se confirmo en el Log que CLON 6 <<< MACD viene enviando mensajes con formato mit barra MIN-TEND en vez de mac barra MACD, es decir que duplica la senal de MIN-TEN en lugar de reportar el MACD real. Esta mal configurada desde su creacion, el lunes 18 de mayo de 2026 a las 12:49:32, y sigue activa disparando cada pocos minutos siempre con la condicion equivocada. El fix es simple, cambiar el dropdown Condicion de esa alerta especifica de MIN TEN CLON6 a MACD V10 V2, pero al ser una edicion real sobre una alerta en produccion se dejo pendiente de autorizacion explicita de Fer antes de aplicarla. No se modifico nada, la revision quedo en modo solo lectura. Ambos hallazgos fueron reportados a Code en la sesion compartida.
